<template>
  <v-card variant="tonal" class="relative w-full overflow-visible" :style="highlight
    ? { border: '2px solid', borderRadius: '0.4rem' }
    : { border: 'none' }
    ...
    <div v-if="similarity" class="absolute left-0 bottom-0 h-full rounded progress-bar -z-100"
      :class="getBarClass(similarity)" :style="{ width: getBarFill(similarity) }" />
  </v-card>
</template>
